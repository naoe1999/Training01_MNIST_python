...
class SimpleMLP:
    ...
    def feedforward(self, x):       # x: input 행렬, shape: (n, 784), n: 데이터 개수
        ...
        h1 = sigmoid(z1)            # h1: 첫번째 hidden layer의 출력 행렬 (활성화)
        z2 = np.dot(h1, W2)         # z2: 두번째 hidden layer의 입력 행렬
        h2 = sigmoid(z2)            # h2: 두번째 hidden layer의 출력 행렬 (활성화)
...
if __name__ == '__main__':
    ...
    # 정확도 얼마까지 나오나요?
    # hyper parameter 조절하면서 학습을 해보세요. (learning rate, sigmoid -> relu, node 수, layer 수 등)
    # 가장 좋은 결과와 그 때의 hyper parameter들을 기록해 보세요.
